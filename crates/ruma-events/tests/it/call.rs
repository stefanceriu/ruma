...
fn candidates_v0_content_serialization() {
    let event_content = CallCandidatesEventContent::version_0(
        "abcdef".into(),
        vec![Candidate::new("not a real candidate".to_owned(), Some("0".to_owned()), Some(uint!(0)))],
    );
...
fn candidates_v1_event_serialization() {
    let content = CallCandidatesEventContent::version_1(
        ...
        "9876".into(),
        vec![
            Candidate::new("not a real candidate".to_owned(), Some("0".to_owned()), Some(uint!(0))),
            Candidate::new("another fake candidate".to_owned(), Some("0".to_owned()), Some(uint!(1))),
        ],
    );
...
fn candidates_v1_event_deserialization() {
    ...
    assert_eq!(content.candidates[0].sdp_mid, Some("0".to_owned()));
    ...
    assert_eq!(content.candidates[1].sdp_mid, Some("0".to_owned()));
